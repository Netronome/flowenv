...
#define NBI_PC_PE_SETUP_ADDR(_nbi)                                    \
...
#define CTMRN_CHAN2PORT_ENTRY_READ(_data, _off)                     \
    ((_data[((_off) >> 2) ^ 0x1] >> (((_off) & 0x3) << 3)) & 0xFF)
#define CTMRN_CHAN2PORT_ENTRY_WRITE(_data, _off, _val)                      \
    do {                                                                    \
        _data[((_off) >> 2) ^ 0x1] &= ~(0xFF << (((_off) & 0x3) << 3));       \
        _data[((_off) >> 2) ^ 0x1] |= (_val & 0xFF) << (((_off) & 0x3) << 3); \
    } while (0)
...
catamaran_cls_hash_init(unsigned int nbi)
{
    ...
    pe_setup.__raw = xpb_read(NBI_PC_PE_SETUP_ADDR(nbi));
...
{
    __lmem uint8_t cfg_entry[NBI_PC_MEM_ENTRY_SIZE];

    /* Store the MAC match hash scalar. */
    nbi_pc_pelm2lmem_copy(nbi, CTMRN_CFG_MAC_MATCH_HASH_ADDR, cfg_entry,
                          sizeof(cfg_entry));
                              ...
                              __lmem struct catamaran_chan2port_entry *entries)
{
    ...
    for (block = block_start; block <= block_end; ++block) {
        ...
        nbi_pc_pelm2lmem_copy(nbi, addr, data, sizeof(data));
                              ...
                              __lmem struct catamaran_chan2port_entry *entries)
{
    ...
    for (block = block_start; block <= block_end; ++block) {
        ...
        nbi_pc_pelm2lmem_copy(nbi, addr_pri, data, sizeof(data));
        ...
        nbi_pc_lmem2pelm_copy(nbi, addr_pri, addr_sec, data, sizeof(data));
                              ...
                              uint16_t result, unsigned int cls_hash_idx)
{
    ...
    nbi_pc_pesm2lmem_copy(nbi, addr0, table_entry.__raw, sizeof(table_entry));
    ...
    if ((mac_addr_hi == table_entry.mac_addr0_hi) &&
        ...
        nbi_pc_lmem2pesm_copy(nbi, addr0, addr1, table_entry.__raw,
                              sizeof(table_entry));
                              ...
                              unsigned int cls_hash_idx)
{
    ...
    /* Retrieve the MAC match entry. */
    nbi_pc_pesm2lmem_copy(nbi, addr, table_entry.__raw, sizeof(table_entry));
...
int
catamaran_lb_table_get(unsigned int nbi, unsigned int lb_start,
                       unsigned int lb_end,
                       __lmem struct catamaran_lb_entry *entries)
{
    ...
    uint32_t addr;
    uint32_t block;
    __lmem uint32_t data[8];
    int lb_cnt               = lb_end - lb_start + 1;
    int first_lb             = lb_start & (CATAMARAN_LB_ENTRIES_PER_LOC - 1);
    uint32_t block_end       = lb_end / CATAMARAN_LB_ENTRIES_PER_LOC;
    uint32_t block_start     = lb_start / CATAMARAN_LB_ENTRIES_PER_LOC;
    unsigned int entry_cnt   = 0;

    try_ctassert(lb_start < CATAMARAN_LB_NUM_ENTRIES);
    try_ctassert(lb_end < CATAMARAN_LB_NUM_ENTRIES);

    /* Check if any lb vals are being retrieved. */
    if (lb_cnt <= 0)
        return -1;

    /* Retrieve the lb vals one block at a time. */
    for (block = block_start; block <= block_end; ++block) {
        /* Retrieve the block of lb vals. */
        addr = (CATAMARAN_LB_TABLE_BASE0 + block) << 4;

        nbi_pc_pelm2lmem_copy(nbi, addr, data, sizeof(data));

        /* Extract the info for each lb val. */
        last_lb =
            (lb_cnt < (CATAMARAN_LB_ENTRIES_PER_LOC - first_lb)) ?
            (first_lb + lb_cnt) : CATAMARAN_LB_ENTRIES_PER_LOC;

        for (lb = first_lb; lb < last_lb; ++lb) {
            /* Store the lb val-to-port entry info. */
            uint32_t subword, wordoff, bitoff;

            wordoff = ((lb / 4) ^ 2) % 8; /* 2 3 0 1 6 7 4 5 */
            bitoff = (3 - (lb % 4)) * 8; /* 3 2 1 0 */
            subword = (data[wordoff] >> bitoff) & 0xff;
            entries[entry_cnt]._reserved = 0;
            entries[entry_cnt].ro_ctx = (subword >> CATAMARAN_LB_SEQ_VAL_SHIFT) & 0x7;
            entries[entry_cnt].nfp = (subword >> CATAMARAN_LB_TNFP_VAL_SHIFT) & 0x3;
            entries[entry_cnt].bp = (subword >> CATAMARAN_LB_BP_VAL_SHIFT) & 0x7;

            ++entry_cnt;
        }

        /* Prepare for reading the lb val info in the next block. */
        first_lb  = 0;
        lb_cnt   -= CATAMARAN_LB_ENTRIES_PER_LOC;
    }
    return 0;
}

int
catamaran_lb_table_set(unsigned int nbi, unsigned int lb_start,
                       unsigned int lb_end,
                       __lmem struct catamaran_lb_entry *entries)
{
    int lb;
    int last_lb;
    uint32_t addr_pri, addr_sec;
    uint32_t block;
    __lmem uint32_t data[8];
    int lb_cnt               = lb_end - lb_start + 1;
    int first_lb             = lb_start & (CATAMARAN_LB_ENTRIES_PER_LOC - 1);
    ...
    for (block = block_start; block <= block_end; ++block) {
        /* Retrieve the block of lb vals. */
        addr_pri = (CATAMARAN_LB_TABLE_BASE0 + block) << 4;
        addr_sec = (CATAMARAN_LB_TABLE_BASE1 + block) << 4;
        ...
        for (lb = first_lb; lb < last_lb; ++lb) {
            /* Store the lb val-to-port entry info. */
            uint32_t subword, val, bitoff, wordoff;

            subword = entries[entry_cnt].ro_ctx << CATAMARAN_LB_SEQ_VAL_SHIFT;
            subword |= entries[entry_cnt].nfp << CATAMARAN_LB_TNFP_VAL_SHIFT;
            subword |= entries[entry_cnt].bp << CATAMARAN_LB_BP_VAL_SHIFT;
